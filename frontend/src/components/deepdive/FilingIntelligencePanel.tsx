  ...
  return (
    <div
      className={`rounded-xl border p-3.5 ${
        t.disclosed ? 'border-[#e5e7eb] bg-white' : 'border-dashed border-[#e5e7eb] bg-[#fafafa]'
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span
          className={`flex-none rounded-full px-2 py-0.5 text-[0.62rem] font-bold uppercase tracking-[0.04em] ${
            t.disclosed ? 'bg-[#ecfdf5] text-[#047857]' : 'bg-[#f1f5f9] text-[#94a3b8]'
          }`}
        >
          {t.disclosed ? 'disclosed' : 'not disclosed'}
        </span>
      </div>
      <p className="mt-1.5 text-[0.85rem] leading-relaxed text-[#1e293b]">{t.finding}</p>
      {t.evidence && (
    ...
    <section className="rounded-card border border-[#e5e7eb] bg-white p-5 shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-1 items-start gap-3">
          ...
          <div>
            <div className="text-base font-bold text-[#111827]">Filing intelligence</div>
            {!collapsed && (
      ...
      {!collapsed && (
        <>
          <div className="mt-3 inline-flex gap-1 rounded-xl bg-[#f8fafc] p-1">
