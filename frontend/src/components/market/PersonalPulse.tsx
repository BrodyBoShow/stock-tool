...
function PulseName({ label, n }: { label: string; n: MarketPulseName }) {
  return (
    <div>
      <div className="text-[0.6rem] font-bold uppercase tracking-[0.08em] text-slate-400">{label}</div>
      <Link
        to={`/securities/${n.ticker}`}
        className="mt-0.5 flex items-baseline gap-1.5 hover:opacity-80"
      >
        <span className="text-[0.9rem] font-bold text-slate-800">{n.ticker}</span>
...
export function PersonalPulse() {
  ...
  if (data.count === 0) {
    return (
      <div className="rounded-card border border-dashed border-slate-200 bg-slate-50/70 px-4 py-3 text-[0.82rem] text-slate-500">
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-card border border-gray-200 bg-white px-4 py-3 shadow-card">
      <div>
        <div className="text-[0.6rem] font-bold uppercase tracking-[0.08em] text-slate-400">
        ...
        <div className="mt-0.5 flex flex-wrap items-baseline gap-x-2">
          ...
          <span className="text-[0.76rem] text-slate-400">
            ...
            {rel != null && (
              <span className={rel >= 0 ? 'text-emerald-600' : 'text-rose-600'}>
                {' · '}
