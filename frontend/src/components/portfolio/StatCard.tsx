...
export function StatCard({
  ...
  return (
    <div className="rounded-card border border-line bg-surface px-4 py-3.5 shadow-card">
      <div className="flex items-center text-[0.68rem] font-semibold uppercase tracking-[0.09em] text-muted">
        <span>{label}</span>
      ...
      {sub && <div className="mt-0.5 text-[0.72rem] text-muted">{sub}</div>}
    </div>
  )
}
